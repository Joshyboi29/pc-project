...
?>
  ...
  <script>
  var x = document.getElementById('vr')
  x.className = "active";
  ...
  <div class="pagetitle">
    ...
    <div class="card-body">
      ...
      <?php  
        ...
        if($count == 0) {
            ...
            if(isset($_POST['search'])) {
                ...
                echo "<td colspan='7' style='text-align:center;'>No request vehicle</td>";
            }
            echo "</tr>";
        } else {
      ...
      <tr class="align=middle text-secondary text-center">
        ...
        <td>
        <button class="btn btn-light border shadow-sm" style="width: 90px; height: 35px;" onclick="approveConfirmation(<?php echo $id;?>)">
          <a style="Color: Green">Approve</a>
        </button>
        <button class="btn btn-light border shadow-sm" style="width: 90px; height: 35px;" onclick="rejectConfirmation(<?php echo $id;?>)">
        ...
        <script>
          function approveConfirmation(id) {
            Swal.fire({
              ...
              if (result.value) {
                window.location.href = 'action/approve/log-approve-rv.php?id=' + id;
              }
            })
          }
        </script>
  ...
  <?php }} ?>
    ...
    <?php
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT COUNT(*) FROM dispatch_vcl_rv WHERE plate LIKE :search");
          $stmt->bindParam(':search', $search);
          $stmt->execute();
