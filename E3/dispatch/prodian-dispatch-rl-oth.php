...
?>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active"><a href="prodian-dispatch-rl.php">Request List</a></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="card-body">
      ...
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
          <a class="nav-link" href="prodian-dispatch-rl-sa.php" role="tab" aria-selected="false" style="color: #256D85;">Supplies</a>
        </li>
        <li class="nav-item" role="presentation">
          <a class="nav-link active" href="prodian-dispatch-rl-oth.php" role="tab" aria-selected="true">Others</a>
        </li>
      </ul>
      ...
      <?php  
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT * FROM dispatch_dp_oth WHERE name LIKE :search ORDER BY id ASC LIMIT :start, :limit");
          ...
          $stmt = $conn->prepare("SELECT * FROM dispatch_dp_oth ORDER BY id ASC LIMIT :start, :limit");
          $stmt->bindParam(':start', $start, PDO::PARAM_INT);
          $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
          $stmt->execute();
          $count = $stmt->rowCount();
      ...
      <div class="modal fade" id="dispatch<?php  echo $id;?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <form method="POST" action="../../action/dispatch/dispatching/prodian-dp-oth.php<?php echo '?id='.$id; ?>" enctype="multipart/form-data" class="row g-12">
              ...
              <div class="row mb-3">
              <label for="branch" class="col-md-4 col-lg-3 col-form-label">Building Branch :</label>
                <div class="col-md-8 col-lg-9">
                  <input name="branch" type="text" class="form-control" value="<?php echo $row['branch'];?>" disabled>
                  <input type="hidden" name="branch" value="<?php echo $row['branch']; ?>">
                </div>
              </div>
              
              <div class="row mb-3">
                <label for="driver" class="col-md-4 col-lg-3 col-form-label">Dispatching Driver :</label>
                <div class="col-md-8 col-lg-9">
                  <div class="form-group">
                    <select name="driver" class="form-select" required>
                      <option>Choose Driver...</option>
                      <?php
                        $stmt2 = $conn->prepare("SELECT * FROM dispatch_dr");
                        $stmt2->execute();
                        while ($result = $stmt2->fetch(PDO::FETCH_ASSOC)) {
                          echo '<option value="' . $result['fname'] . ' ' . $result['lname'] . '">' . $result['fname'] . ' ' . $result['lname'] . '</option>';
                        }
                      ?>
                    </select>
                  </div>
              ...
              <div class="row mb-3">
                <label for="vehicle" class="col-md-4 col-lg-3 col-form-label">Dispatching Vehicle :</label>
                <div class="col-md-8 col-lg-9">
                  <div class="form-group">
                    <select name="vehicle" class="form-select" required>
                      <option>Choose Vehicle...</option>
                      <?php
                        $stmt3 = $conn->prepare("SELECT * FROM dispatch_vcl_bv");
                        $stmt3->execute();
  ...
  <?php }} ?>
    ...
    <?php
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT COUNT(*) FROM dispatch_dp_oth WHERE req_name LIKE :search");
          $stmt->bindParam(':search', $search);
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        } else {
          $stmt = $conn->prepare("SELECT COUNT(*) FROM dispatch_dp_oth");
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        }
