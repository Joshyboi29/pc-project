...
?>
    ...
    <div class="card-body">
      ...
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
          <a class="nav-link" href="prodian-dispatch-dp-sa.php" role="tab" aria-selected="false" style="color: #256D85;">Supplies</a>
        </li>
        <li class="nav-item" role="presentation">
          <a class="nav-link active" href="prodian-dispatch-dp-oth.php" role="tab" aria-selected="true">Others</a>
        </li>
      </ul>
      ...
      <script>
        ...
        ?>
          Swal.fire({
            title: "Delivered!",
            text: "The assets request has been Delivered.",
            icon: "success",
      ...
      <?php  
        ...
        $stmt = $conn->prepare("SELECT * FROM dispatch_dp_oth_done ORDER BY id ASC LIMIT :start, :limit");
      ...
      <tr class="align=middle text-secondary text-center">
        ...
        <td><?php echo $row['vcl'];?></td>
        <td><?php echo $row['driver'];?></td>
        <td><?php echo $row['branch'];?></td>
        <td>
        <button class="btn btn-light border shadow-sm" style="width: 125px; height: 40px;" onclick="Confirmation(<?php echo $id;?>)">
          <a style="Color: #256D85"><i class="bi bi-check2-circle"></i> Delivered</a>
        </button>
        </td>
        ...
        <script>
          function Confirmation(id) {
            Swal.fire({
              title: 'Are you sure?',
              text: 'Do you sure that the assets has been Delivered?',
              icon: 'warning',
              ...
              if (result.value) {
                window.location.href = '../../action/dispatch/dispatching/prodian-dp-oth-done.php?id=' + id;
              }
            })
          }
        </script>
  ...
  <?php }} ?>
    ...
    <?php
      ...
      $stmt = $conn->prepare("SELECT COUNT(*) FROM dispatch_dp_oth_done");
      $stmt->execute();
