...
?>
  ...
  <div class="pagetitle">
      ...
      <tbody>
        <?php  
          ...
          if(isset($_POST['search'])) {
            $search = "%" . $_POST['search'] . "%";
            $stmt = $conn->prepare("SELECT * FROM prodian_mro WHERE rname LIKE :search OR rpos LIKE :search ORDER BY id ASC LIMIT :start, :limit");
        ...
        <tr class="align=middle text-secondary text-center" ondblclick="showModal(<?php echo $id;?>)">
          ...
          <td>
          <button class="btn btn-light border shadow-sm" style="width: 90px; height: 40px;">
            <a href="action/approve/approve-ar.php?id=<?php echo $id;?>" style="Color: Green">Approve</a>
          </button>
          <button class="btn btn-light border shadow-sm" style="width: 90px; height: 40px;">
    ...
    <?php
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT COUNT(*) FROM prodian_mro WHERE rname LIKE :search OR rpos LIKE :search");
          $stmt->bindParam(':search', $search);
          $stmt->execute();
        ...
        <ul class="pagination">
          <li class="page-item <?php if($page <= 1) { echo 'disabled'; } ?>">
            <a class="page-link" href="prodian-mro.php?page=<?php echo $page-1; ?>" aria-label="Previous">
        ...
        <?php for($i = 1; $i <= $total_pages; $i++) { ?>
          <li class="page-item <?php if($page == $i) { echo 'active'; } ?>" style="<?php if($page == $i) { echo 'color: black;'; } ?>">
            <a class="page-link" href="prodian-mro.php?page=<?php echo $i; ?>">
              <?php echo $i; ?>
            </a>
          </li>
        <?php } ?>
          <li class="page-item <?php if($page >= $total_pages) { echo 'disabled'; } ?>">
            <a class="page-link" href="prodian-mro.php?page=<?php echo $page+1; ?>" aria-label="Next">
    ...
    <div class="row">
        ...
        <div class="row">
          <div class="col-12">
            <div class="card asset-list  overflow-auto">
              ...
              <div class="card-body">
                <h5 class="card-title">Check-up Assets</h5>
                <table class="table table-borderless table-hover table-responsive">
                  <thead>
                    <tr class="align=middle text-center">
                      <th scope="col">Requestor</th>
      ...
      <div class="col-lg-4">
        <div class="card asset-list">
          <div class="card-body">
            <h5 class="card-title">Repair Assets</h5>
            <table class="table table-borderless table-hover table-responsive">
            <thead>
              <tr class="align=middle text-center">
                <th scope="col">Name of Assets</th>
            ...
            <tbody>
              <tr class="align=middle text-secondary text-center">
                <td>Brandon Jacob</td>
                <td><span class="badge bg-danger text-light">Repairing</span></td>
              </tr>
            </tbody>
          </table>
          </div>
