...
?>
  ...
  <div class="pagetitle">
    ...
    <div class="card-body">
      ...
      <h5 class="card-title">Dispatch History</h5>
      ...
      <?php  
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT * FROM prodian_dispatch WHERE name LIKE :search ORDER BY id ASC LIMIT :start, :limit");
          ...
          $stmt = $conn->prepare("SELECT * FROM prodian_dispatch ORDER BY id ASC LIMIT :start, :limit");
          $stmt->bindParam(':start', $start, PDO::PARAM_INT);
          $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
          $stmt->execute();
          $count = $stmt->rowCount();
        ...
        if($count == 0) {
            ...
            if(isset($_POST['search'])) {
                ...
                echo "<td colspan='6' style='text-align:center;'>No Dispatch Assets</td>";
            }
            echo "</tr>";
        } else {
  ...
  <?php }} ?>
    ...
    <?php
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT COUNT(*) FROM prodian_dispatch WHERE req_name LIKE :search");
          $stmt->bindParam(':search', $search);
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        } else {
          $stmt = $conn->prepare("SELECT COUNT(*) FROM prodian_dispatch");
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        }
