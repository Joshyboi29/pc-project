...
?>
  ...
  <script>
  var x = document.getElementById('poa')
  x.className = "active";
    ...
    <div class="card-body">
      ...
      <?php  
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT * FROM inventory_poa WHERE name LIKE :search ORDER BY id ASC LIMIT :start, :limit");
          ...
          $stmt = $conn->prepare("SELECT * FROM inventory_poa ORDER BY id ASC LIMIT :start, :limit");
          $stmt->bindParam(':start', $start, PDO::PARAM_INT);
          $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
          $stmt->execute();
          $count = $stmt->rowCount();
        ...
        if($count == 0) {
            ...
            if(isset($_POST['search'])) {
                ...
                echo "<td colspan='4' style='text-align:center;'>No Pull Out Assets</td>";
            }
            echo "</tr>";
        } else {
  ...
  <?php }} ?>
    ...
    <?php
        ...
        if(isset($_POST['search'])) {
          $search = "%" . $_POST['search'] . "%";
          $stmt = $conn->prepare("SELECT COUNT(*) FROM inventory_poa WHERE req_name LIKE :search");
          $stmt->bindParam(':search', $search);
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        } else {
          $stmt = $conn->prepare("SELECT COUNT(*) FROM inventory_poa");
          $stmt->execute();
          $total_records = $stmt->fetchColumn();
        }
        ...
        ?>
          ...
          <ul class="pagination">
            <li class="page-item <?php if($page <= 1) { echo 'disabled'; } ?>">
              <a class="page-link" href="prodian-inventory-poa.php?page=<?php echo $page-1; ?>" aria-label="Previous">
          ...
          <?php for($i = 1; $i <= $total_pages; $i++) { ?>
            <li class="page-item <?php if($page == $i) { echo 'active'; } ?>">
              <a class="page-link" href="prodian-inventory-poa.php?page=<?php echo $i; ?>">
                <?php echo $i; ?>
              </a>
            </li>
          <?php } ?>
            <li class="page-item <?php if($page >= $total_pages) { echo 'disabled'; } ?>">
              <a class="page-link" href="prodian-inventory-poa.php?page=<?php echo $page+1; ?>" aria-label="Next">
