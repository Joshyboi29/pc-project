...
?>
  ...
  <div class="pagetitle">
    ...
    <div class="card-body">  
      ...
      <?php  
        ...
        $stmt = $conn->prepare("SELECT * FROM mro_repair_request ORDER BY id ASC LIMIT :start, :limit");
      ...
      <tr class="align=middle text-secondary text-center">
        ...
        <td>
        <button class="btn btn-light border shadow-sm" style="width: 130px; height: 35px;" onclick="approveConfirmation(<?php echo $id;?>)">
          <a style="Color: Green"><i class="bi bi-clipboard-check" style='color: green;'></i> Check-up</a>
        </button>
        </td>
        ...
        <script>
          function approveConfirmation(id) {
            Swal.fire({
              ...
              if (result.value) {
                window.location.href = '../../action/mro/checking/mro-request.php?id=' + id;
              }
            })
          }
        </script>
  ...
  <?php }} ?>
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM mro_repair_request");
    $stmt->execute();
    ...
    <nav style="float: right; margin-top: 5px">
      <ul class="pagination">
        <li class="page-item <?php if($page <= 1) { echo 'disabled'; } ?>">
          <a class="page-link" href="prodian-mro-request.php?page=<?php echo $page-1; ?>&limit=<?php echo $limit; ?>" aria-label="Previous">
        ...
        <li class="page-item <?php if($page == $i) { echo 'active'; } ?>">
          <a class="page-link" href="prodian-mro-request.php?page=<?php echo $i; ?>&limit=<?php echo $limit; ?>">
            <?php echo $i; ?>
          </a>
        </li>
        <?php } ?>
        <li class="page-item <?php if($page >= $total_pages) { echo 'disabled'; } ?>">
          <a class="page-link" href="prodian-mro-request.php?page=<?php echo $page+1; ?>&limit=<?php echo $limit; ?>" aria-label="Next">
